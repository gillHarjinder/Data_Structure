...
//PURPOSE: pops all elements from the stack to make it empty if it is not empty yet.
 void stack::clearIt(){
     int pop_elem;           //  pop_elem is an int value which is about to pop from the stack

     while(!isEmpty()){
            pop(pop_elem);   // This while loop is pop the stack element one by one until the stack is empty
        }
